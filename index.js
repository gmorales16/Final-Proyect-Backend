...
app.put("/auth/review/:isbn", (req, res) => {
  ...
  if (!existing_review) {
    ...
    return res.send(
      `this is the first review -  ${JSON.stringify(books[isbn].reviews)}`
    );
  } else {
    if (existing_review.user === user) {
      ...
      return res.send(
        `you have updated your review -  ${JSON.stringify(books[isbn].reviews)}`
      );
    } else {
      books[isbn].reviews[user] = {
        review: review,
      ...
      return res.send(
        `there is a review  ${JSON.stringify(books[isbn].reviews)}`
      );
    }
  }
});
...
app.delete("/auth/review/:isbn", (req, res) => {
  ...
  if (!existing_review) {
    ...
    if (existing_review.user === user) {
      ...
      return res.send(
        `Your review has been deleted - ${JSON.stringify(books[isbn].reviews)}`
      );
    } else {
